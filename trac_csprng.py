"""
TRAC — Cliente CSPRNG compartilhado (Hash de Caos)
Centraliza a geração de seeds pseudoaleatórias e conversão para valores float/int/bool.

Nota: Este é um CSPRNG (Cryptographically Secure Pseudo-Random Number Generator),
não um TRNG real. Usa algoritmo determinístico (Hash de Caos) com entropia do sistema.
"""
...
class CSPRNGClient:
    """Cliente para comunicação com servidor CSPRNG (porta 9999).
    
    Fornece métodos estáticos para gerar seeds pseudoaleatórias baseadas em
    hash de caos e conversão para tipos.
    Fallback: se CSPRNG offline, usa time.perf_counter_ns().
    """

    @staticmethod
    def get_seed(jitter: int = 0) -> int:
        """Obtém uma seed de 32 bits do servidor CSPRNG.
